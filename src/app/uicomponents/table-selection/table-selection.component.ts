    ...
    if (this.select) {
      if (this.allSelected) {
        ...
        this.select.options.forEach((item: MatOption, index) => {
          if (item.value === 'TelephoneNumber' || item.value === 'View' || item.value === 'Link' || item.value === 'Links' || item.value === 'StartTelephoneNumber' || (this.reportIdentifier === 'AuditStatusTracker' && item.value ==='ActId') || (this.reportIdentifier === 'AuditUserActionSummary' && item.value ==='AuditACTID' )) {
            //  item.deselect();
            ;
          } else {
            item.deselect();
